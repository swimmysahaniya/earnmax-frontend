    ...
    //print_r($extra_earning);die;
        ...
        if ($extra_income_count == 0 && $extra_earning > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO myapp_extraincome (user_mobile_id, extra_amount, status, created_at) VALUES (?, ?, '0', NOW())");
    ...
    function updateWalletBalance($conn, $user_mobile, $withdraw_amount = 0) {
        ...
        $stmt = $conn->prepare("
            SELECT
                ...
                COALESCE((
                    ...
                    WHERE e.user_mobile_id = u.mobile AND e.status = '1'
                ), 0) +
                COALESCE((
        ...
        if (!$wallet_exists) {
            ...
            $stmt = $conn->prepare("
                INSERT INTO myapp_wallet (user_mobile_id, balance, total_withdrawn, tds_earning, updated_at)
                VALUES (?, ?, ?, ?, NOW())
            ");
    ...
    if ($user_balance > 0) {
        ...
        file_put_contents("withdraw_debug.txt", json_encode($_POST) . PHP_EOL, FILE_APPEND);
    ...
    $query = "SELECT extra_amount FROM myapp_extraincome WHERE user_mobile_id = '$user_mobile' AND status = '1'";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);
    ...
    // Fetch user_referral_code
    $query = "SELECT referral_code FROM myapp_users WHERE mobile = '$user_mobile' AND (status = '1' OR status = '3')"; // Only approved users or locked users
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);
    $user_referral_code = $row['referral_code'] ?? 0;
    //print_r($user_referral_code);die;
    ...
    <div class="container mt-5">
        ...
        <br>
        <h4 style="color:red; text-align:center;">Note: Users who do not withdraw their total earnings for <br> 6 consecutive months will earn an additional 10% incentive.</h4>
        <br>
        <p>Available Balance: ₹ <?php echo number_format($total_wallet, 2); ?></p>
        <form method="post">
            ...
            <table class="table">
                ...
                <?php foreach ($withdrawals as $withdrawal) { ?>
                    <tr>
                        ...
                        <td><?php echo "₹ " . number_format($withdrawal['tds_amount'], 2); ?></td>
